...
and is naturally related to the amount allowed perturbations on input
data. As a consequence, when using
...
\txtblue{In this paper, we first present the
...
In~\cite{DGCI2013}, \comJeremy{authors define a  2D digital curvature estimator $\CurvH{R}$
and a 3D digital mean curvature estimator $\MeanCurvH{R}$  based on the
digital volume estimator $\VolC(Y,h) \EqDef h^d \MCard(Y)$ (area estimator $\AreaC(Y,h)$ in dimension 2):}
%% Redite
...
\end{Definition}
%
\comJeremy{Such curvature estimators have multigrid convergence
  properties~\cite{DGCI2013}:} For short, when
the digital object becomes finer and finer, \ie when the digitization
step $h$ tends to zero, the estimated quantity on
$\Bd{\Body{\DSh}{h}}$ converges (theoretically and experimentally) to
the \txtblue{ associated one on $\Bd{\Shape}$ in
  $O\left(h^{\frac{1}{3}}\right)$ for convex shapes with at least
  $C^3$-boundary and bounded curvature (setting $R\EqDef
  kh^\frac{1}{3}$ for some $k\in\R$).}
%
...
%\sout{ Still, this method is not fully
