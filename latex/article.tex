...
we get consequently much bigger errors. Those levels
...
The GPU's interpolation reduces the bias, but it doesn't remove it.
...
estimation due to the huge amount of data to process. By computing a
...
(in logarithmic scale) for a triangulation that is dynamically refining according
...
Another advantage with approximation, is that it allows us to get a visualisation of our object as soon as we run
...
It is also visible in Figure~\ref{fig:timings} that using a hierarchical decomposition greatly reduces the curvature computation
...
%it only needs 700ms. 
However, due to the precomputation and current hierarchical
