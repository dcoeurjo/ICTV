...
%\address[liris]{Universit\'e de Lyon, CNRS, INSA-Lyon, LIRIS, UMR5205, F-69622, France}
...
\comJeremy{Such curvature estimators have multigrid convergence
  properties~\cite{DGCI2013}:} when the digital object becomes finer
and finer, \ie when the digitization step $h$ tends to zero, the
estimated quantities on $\Bd{\Body{\DSh}{h}}$ converges (theoretically
and experimentally) to the \txtblue{ associated one on $\Bd{\Shape}$
  in $O\left(h^{\frac{1}{3}}\right)$ for convex shapes with at least
