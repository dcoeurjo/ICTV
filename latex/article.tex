...
The proposed approch uses an octree representation of the input object
...
to the GPU: Given a set of cells (a vector of morton codes),
each triangle patch can be extracted in parallel for both the regular cell and the transition cells.
...
independently on all cells.
%Hence, we can efficiently determine if a cell must be subdivided or not.
Figure~\ref{fig_lod_octree} illustrates our the level of
details (LoD for short) criterion% we use. More precisely,
...
Using such a criterion, split and merge decisions are computed in%of all cells are computed in
parallel from all cell's morton codes.
