...
\begin{figure}
  ...
  \end{center}
  \caption{\emph{Third row:} Mean, Gaussian, first and second principal curvature estimation on an OctaFlower with a digital domain of $130^3$.
...
Figure~\ref{fig:full}-top and middle show results of curvature tensor estimation
(mean, Gaussian curvature, first and second principal curvatures, first and
second principal directions) on an OctaFlower. Figure~\ref{fig:full}-bottom
shows the mean curvature estimation in real-time on a dynamic object. For all
illustrations, we use directly normal vectors computed by algorithm discussed in
Section~\ref{sec:curv-tens-estim}.

We compare the approximation made to accelerate visualisation with the ground
truth result obtained at level $l_0$. We also compare it with the hierarchical
algorithm. The results are presented Table~\ref{tab:full-res-stat}, and
illustrated Figure~\ref{fig:refine}.
...
Bare in mind that those results were measured on a full resolution triangulation.
...
%
    \begin{tikzpicture}[scale=0.7]
     ...
     \end{tikzpicture}
     \end{center}
  \caption{Timings obtained while visualising an adaptive triangulation of an object (left) computing the curvature with a regular grid and (right) computing it with the hierarchical algorithm. 		
  Orange is the required time to extract the triangulation, and blue are the curvature computations from level $l_4$ (light blue) to level $l_0$ (drak blue).
  We measured three datasets, OctaFlower (digital domain of $130^3$ ), snow micro structures($233^3$) and the XYZ Dragon ($510^3$), with different radii (8, 10, 16, 20)}
  \label{fig:timings}
\end{figure}

In this experiments, we measure the timings obtained when running our framework on a
GeForce GTX 850M. Here, the triangulation is dynamically refining according to its distance to the camera.
At each frame, both the curvature and the triangulation are recomputed.
If the visualisation stays unchanged, we compute the curvature at a level $l-1$ until the level $l_0$ is reached.
We measured those timings using the regular and the hierarchical algorithm. The results are presented in 
logarithmic scale figure \ref{fig:timings}.

We can first note that the required time for an object to compute ground truth curvature is usually
as high as the time required to extract the geometry and compute all the above levels.
Using approximations is thus mandatory, since for big objects such as the XYZ Dragon (Digital Domain $510^3$)
with high radii, the computation is seconds faster, the $l_0$ visualisation takes more than 4 seconds to compute, whereas
the approximated visualisation at $l_2$ takes only 300ms.
We can thus get an approximated visualisation of our object as soon as we run the application, allowing for
real time interactions with the object (changing the visualised value, the curvature radius, etc ...).
Once an interesting portion if the object is reached, it only takes a few seconds to the application to display
ground truth curvature estimation.

It is also clear on those graphs that using a hierarchical decomposition reduces the curvature computation time.
For the XYZ Dragon with a radius of 20, it needs almost 4 seconds to compute the $l_0$ level, whereas using the
hierarchical algorithm, it only needs 700ms. However, due to the precomputation, this algorithm is biased and
creates an error (presented in Table~\ref{tab:full-res-stat}) that needs to be considered


\subsection{Visualisation}

For dynamic datasets, we consider a time varying implicit
