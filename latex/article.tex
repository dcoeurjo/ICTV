...
%
% This file and related figures are under Creative Commons CC BY-NC-SA 2.0
...
\newcommand{\wnlog}{w.l.o.g.\xspace}
...
\txtblue{
Here, we focus on curvature tensor estimation (mean/Gaussian curvature, principal curvatures
directions, \ldots). Most curvature estimators require a parameter fixing the scale
...
and is naturally related to the amount of perturbation on input
data\todo{DC: ``related to a expected signal/noise ratio?''}. As a consequence, when using
such estimators, exploring different values of this parameter is mandatory.
However, this is usually an offline process, due to the amount of computations that
need to be done.}
%In this work, we present an interactive visualization of volumetric data isosurfaces combined with
...
This system allows us to visualize
...
triangulated mesh resolution according to the object geometry or the
...
\cite{lengyel2010voxel} suits best our needs.
%However, it
...
Our high framerate allows us to inspect dynamic 3D data in real-time.
...
\vspace{0.2cm} \txtblue{In this paper, we first present the
previous works on curvature tensor estimation. Then, we propose an
efficient GPU approach to extract a triangulated isosurface from a
digital object. Finally, we propose a fully-parallel  GPU pipeline to compute
curvature tensor in real-time and present our results.}
...
Gauss digitization of \comJeremy{a shape $\Shape \subset \R^d$ } in a $d-$dimensional grid with grid step $h$,
...
In~\cite{DGCI2013}, \comJeremy{the 2D digital curvature estimator $\CurvH{R}$
and the 3D digital mean curvature estimator $\MeanCurvH{R}$ are based on the
digital volumetric integral $\VolC(Y,h) \EqDef h^d \MCard(Y)$ (we denote such
quantity $\AreaC(Y,h)$ in dimension 2):}
%% Redite
%% \Jaco{Given some point $\vx \in \R^d$, the set $Y$ is the digital intersection of $\DigShape$ with the ball of radius $R/h$ and center $\vx/h$.}
%% where $Y$ is the intersection of $\DigShape$ with a ball
%% $\Ball{R/h}{\vx/h}$ with $\MCard(Y)$, the number of digital points of
%% the intersection between a ball $\Ball{R/h}{\vx/h}$ of digital radius
%% $R/h$ and $\DigShape$ (we denote such quantity $\AreaC(Y,h)$ in
%% dimension 2):
%
\begin{Definition}
  %
...
\end{Definition}
%
\comJeremy{Estimators of both 2D curvature and 3D mean curvature on digital
objects with volumetric integral information are proved to be multigrid
convergent~\cite{DGCI2013}.}
%
For short, when the digital object becomes finer and finer, \ie when the
digitization step $h$ tends to zero, the estimated quantity on
$\Bd{\Body{\DSh}{h}}$ converges (theoretically and experimentally) to the
\txtblue{ associated one on $\Bd{\Shape}$ in $O\left(h^{\frac{1}{3}}\right)$ for
convex shapes with at least $C^3$-boundary and bounded curvature (setting
$R\EqDef kh^\frac{1}{3}$ for some $k\in\R$).}
%
...
\begin{Definition}
  ...
  of the covariance matrix of \comJeremy{$\Ball{R/h}{\vx/h} \cap Z$.}
  % the intersection between the digital shape
...
%%% Changer avec le chapitre de Jaco à un moment
...
% from hierarchical octree cells.
...
Furthermore, cell operations such as subdivision, merging %, neighbors access
...
%Since triangle patches are locally computed,
...
%extension  of MC configurations to handle adaptive structures
algorithm
...
Similarly to original MC algorithm, this approach is well suited %perfectly adapted
...
%(done by the \emph{geometry shader} in the GPU graphic pipeline).
...
%Hence, we can efficiently determine if a cell must be subdivided or not.
...
and its projected size. %$2\cdot d_c\cdot\tan(\alpha)$, $d_c$ being the distance between the cell
...
\begin{figure}[!h]
  ...
  %Based on LoD criterion evaluation, the cell buffer is updated between each frame \emph{(right)}.
...
%Updating the cell buffer can be
...
\txtblue{
Since GPU have massively parallel architectures, we can do all those computations in parallel
to obtain a very efficient implementation. Please note that}
...
\txtblue{To implement the integration on $\Ball{R}{\vx} \cap \Shape$
  (Fig.~\ref{fig:notations}), several strategies have been evaluated.}

\subsection{Per Vertex Real-time Computation on GPU}
\label{sec:hierarchical}

\paragraph{Naive Approach.}
\txtblue{A first simple solution  consists in scanning all digital
points, at a given resolution, lying inside the integration domain}
(Fig.~\ref{fig:approx}-$left$) and then \txtblue{estimating} the
...
\txtblue{
This is exactly similar to what is done on the CPU\@. On  the GPU, we can
exploit \emph{mipmap} textures to obtain  multi-resolution information.}
...
\txtblue{
a given level $l$, a texture probe at  a point $(x,y,z)$ returns the fraction of $\DSh$ belonging}
...
\begin{figure}
  \vspace{-0.35cm}
  \begin{center}
    \raisebox{-0.5\height}{\includegraphics[width=4.0cm]{figs/approx-naive}}
    \raisebox{-0.5\height}{\includegraphics[width=4.0cm]{figs/approx-hierarchical}}
    \raisebox{-0.5\height}{\includegraphics[width=4.0cm]{figs/approx-dynamic}}
  \end{center}
  \vspace{-0.35cm}
  \caption{\txtblue{
  	Here are the three approaches we used to evaluate the integrand.
  	(\emph{left}) is the naive approach, where we sum all the digital points that are inside the integrand domain
  	(\emph{middle}) shows the hierarchical decomposition of the ball, thus limiting the number of texture probes to do.
  	(\emph{right}) illustrates the dynamic approach, we probe the textures at a higher resolution and we refine it at each frame until we reach the finer level.
    }}\label{fig:approx}
  \vspace{-0.35cm}
\end{figure}
...
%    %(\emph{left}), or a decomposition of $\Ball{R}{\vx}\cap\Shape$ (\emph{right}).}
%  \label{fig:approx2}
%  \vspace{-0.35cm}
%\end{figure}


\txtblue{\sout{We describe here two different visualization pipeline on GPU that will
...
%(Fig.~\ref{fig:approx}-$middle$)
%or $\Ball{R}{\vx}\cap\Shape$
%(Fig.~\ref{fig:approx2}-$b$).
The idea is to decompose the
ball into mipmap cells of different resolution in order to limit
the number of texture access (important bottleneck on GPU hardwares)
and to get better approximated quantities.
%Note that the scenario in
...
%$\Ball{R}{\vx}\cap\Shape$.
\txtred{On the GPU, computing a hierarchical decomposition of
  $B_R(\vx)$ at each point $\vx$ is highly inefficient since the
  hardware optimizes the parallelism only when the micro-program
  described in the shader has a predictive execution flow. Hence,
  implementing the recursive algorithm (or its de-recursified version)
  requires a lot of branches (conditional \emph{if} tests) in the
  flow.}  We have thus considered a fast multi-resolution
approximation as illustrated in Fig.~\ref{fig:approx}-$middle$: for a
given radius $R$, we precompute a hierarchical decomposition of
$B_R$. Such decomposition is made of octree cells at different
resolutions. At a given MC vertex $\vx$, the algorithm becomes simple
since we just scan the ball cells and estimate the area (or other
moments) from the mipmap values associated to this cell. Note that
these \txtred{precomputed} cells in the $B_R$ decomposition may not be aligned
with mipmap cells. However, we can use GPU %mipmap functionalities
which interpolates mipmap values if we probe at non-discrete
positions. For a given radius $R$, the expected number of cells in
$B_R$ is in $O(\log{R})$. Implementation details on the hierarchical
octree representation can be found in the supplementary material (see
Section~\ref{sec:discussion}). \sout{ Still, this method is not fully
exploiting the GPU parallelism.  The hardware optimizes the
parallelism only when the micro-program described in the shader has a
predictive execution flow. Hence, implementing this recursive
algorithm (or its de-recursified version) requires a lot of branches
(conditional \emph{if} tests) in the flow.}\todo{la discussion vient apres}}
...
%We see that levels close to $l_0$
%lead very few error% than $l_0$,
%and should be considered for exploring our datasets
...
%Those results were measured on a full resolution triangulation.
%However, since we compute the curvature of each vertices of the final
%mesh, a highly triangulated object will be much slower to process.
...
%large objects such as the XYZ-Dragon ($510^3$) with big radii the approximate
...
%For the XYZ-Dragon with $R=20$, it needs almost
...
%
  ...
  triangulation on three objects -- ``OctaFlower'' (digital domain of $130^3$), ``Snow microstructures'' ($233^3$) and ``XYZ-Dragon'' ($510^3$) --  by computing the
...
%probes. We could also use texture streaming to compute on the fly on the CPU
...
%hierarchical pattern presented in Figure\ref{fig:approx2}-$b$. This would prevent
%bias from adding cells of density 1 but only partly convered by the
%ball.
