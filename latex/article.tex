...
l_0$ in order to accelerate visualization with the ground truth result obtained
...
the $l_0$ level. Thanks to this low error, higher approximation levels are visually
very similar to the $l_0$ level. We can therefore use them for exploring our
datasets in real time, changing the radii on the fly and visualizing dynamic datasets.
...
%
    \begin{tikzpicture}[scale=0.7]
     ...
     \end{tikzpicture}
     \end{center}
  \caption{Timings obtained while visualizing an adaptive triangulation of an object (left) computing the curvature with a regular grid and (right) computing it with the hierarchical algorithm.
  Orange is the required time to extract the triangulation, and blue are the curvature computations from level $l_4$ (light blue) to level $l_0$ (dark blue).
  We measured three datasets, OctaFlower (digital domain of $130^3$ ), snow micro structures($233^3$) and the XYZ Dragon ($510^3$), with different radii ($8$, $10$, $16$ and $20$)}
  \label{fig:timings}
\end{figure}
