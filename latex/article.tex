...
present a complex CPU implementation.\todo{DC: rajouter une phrase par
  rapport à lengyel, Helene ? JP?}  In this paper we propose a GPU
...
\sloppy In our context, we consider digital shapes (any subset $Z$ of
$\Z^d$) and boundaries of digital shapes $\partial Z$. We denote by $\DSh$ the
...
but all computations are performed on the digital object $Z$ and thus
estimators
...
To implement the integration on $B_R(\vx)\cap\Shape$\todo{fix Z vs X}
...
a given level $l$, a voxel $(x,y,z)$ contains the fraction of $Z$
...
\begin{figure}
  \begin{center}
    \subfigure[]{\includegraphics[width=5cm]{figs/approx-bh}}
    \subfigure[]{\includegraphics[width=5cm]{figs/approx-ada}}
  \end{center}
