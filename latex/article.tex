...
needs\todo{DC: needs->need?} to be done.
\normalcolor
...
curvature tensor in real time and discuss on experiments. \todo{DC:
  bof le discuss}
\normalcolor
...
\txtblue{To implement the integration on $\Ball{R}{\vx} \cap \Shape$
  (Fig.~\ref{fig:approx}-$a$), several strategies have been evaluated.}
...
(Fig.~\ref{fig:approx}-$b$) and then \txtblue{estimating} the
...
\begin{figure}
  ...
  \begin{center}
    ...
    %TODO Image alignement cellules mipmaps
  \end{center}
  \vspace{-0.35cm}
...
approximation as illustrated in Fig.~\ref{fig:approx2}-$a$: for a
