...
(mean \Jaco{squared errors} of all vertices), as well the number of
...
The GPU interpolation reduces the bias, but it does \Jaco{not} remove it.
...
When dealing with large datasets, we \Jaco{cannot} expect real-time curvature tensor
estimation if we consider the full resolution geometry, due the the huge amount of data \Jaco{to} process. By computing a
...
\Jaco{First} we can note that the required time to
compute the ground truth curvature \Jaco{for an object} is usually as high as the time
...
with approximations is that it allows us to get a visualisation of
...
the visualized \Jaco{quantity}, curvature radius, etc.
