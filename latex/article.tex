...
\begin{abstract}\sloppy
  ...
  \Jaco{at} which the computation is performed and visualized.
...
\Jaco{at} which the computation is performed. For short, such parameter
(integration radius, convolution kernel size, \ldots) specifies \Jaco{a scale for analyzing the object surface, and is naturally related to the amount of perturbation on input data}. As a consequence, exploring interactively
...
\Jaco{triangulated} mesh resolution \Jaco{according to the object geometry or the
...
\cite{lengyel2010voxel} \Jaco{suits best our needs}.
%However, it
...
inspired \Jaco{by} this method,
%using a GPU-friendly Lod criterion
\Jaco{that maintains} a view dependent triangulation.
...
Our high framerate allows \Jaco{us} to inspect dynamic 3D data in real-time.
...
a \Jaco{ball} kernel of a given radius. Additionally, authors have
...
%, in real-time,
such curvature \Jaco{fields} in real-time and
%allowing us
...
the digital volumetric integral $\VolC(Y,h) \EqDef h^d \MCard(Y)$ \Jaco{(we denote such
...
\end{Definition}
%
They have proved that estimating both 2D curvature and 3D mean curvature on digital
...
$\hat{\lambda}_2$ of the covariance matrix \Jaco{are} principal
curvature direction \Jaco{estimators} $\PrincDirH{1}{R}$ and
...
to a GPU implemntation: given a set of cells (a vector of morton codes),
...
independently on \Jaco{every} cell.
...
\begin{figure}[!htbp]
  ...
  \caption{\Jaco{GPU pipeline summary}. Data buffers are
...
GPU at each vertex of the generated \Jaco{triangulated} mesh. First note that
...
Section~\ref{sec:curv-tens-estim} is consistent with the \Jaco{triangulated}
surface obtained on GPU: triangles are used for visualization purposes
but all computations are performed on the digital object $\DSh$ \Jaco{for}
estimators
...
mimic a \Jaco{blend ?} of adaptive Gauss digitization process.
...
$\Ball{R}{\vx}\cap\Shape$. However, on the GPU %hardware,
this approach is
...
cells \Jaco{in} $B_R$ is in $O(\log{R})$. Implementation details on the
...
becomes trivial (simple triple loop) and a lot of texture \Jaco{fetches} are
...
\item If the current framerate is above a given threshold, we decrease
  ...
  reached yet.
\end{enumerate}
A consequence of the multi-pass approach is that \Jaco{when} there is no
...
Figure~\ref{fig:full}-top shows results of curvature tensor
estimation (mean, Gaussian, first and
second principal directions) on an OctaFlower at level $l_0$ (considered as our ground truth in the following).
...
seems to quickly converge to the ground truth results.
Table~\ref{tab:full-res-stat} shows numerical results of $L_\infty$ error (maximal absolute difference for all vertices )
and $L_2$ error (mean absolute error of all vertices), as well the number of \emph{mipmap texture}
fetches. We can also see that the number of texel fetch, required to compute the
curvature, reduces drastically when computing approximations. However, at higher levels,
approximation errors become more important. Those levels
...
The GPU interpolation reduces the bias, but it doesn't remove it.
...
estimation if we consider the full resolution geometry, due the the huge amount of data process. By computing a
...
We can first note that the required time for an object to compute the ground truth
curvature is usually as high as the time required to extract the geometry and to
compute all of the above levels. Using approximations is thus mandatory.
Another advantage with approximations, is that it allows us to get a visualisation of our object as soon as we run
the application. This allows for real-time interactions with the object, required in order to change
the visualized value, curvature radius, \ldots
