...
identique a compute_tutorial2.cpp, mais le compute shader utilise la memoire partage accessible a un groupe de threads.
...
{
    gk::glsl::vec3 p[B];  
    gk::glsl::vec3 edge1[B]; 
    gk::glsl::vec3 edge2[B]; 
    gk::glsl::vec3 color[B]; 
};
...
public:
    ...
    int init( )
    {
        ...
        program= gk::createProgram("triangle_soa.glsl");
        ...
        // recupere les triangles de l'objet et les organise par sequence de 256
        const int max_batch= 256;
        std::vector< TriangleBatch<max_batch> > triangles;
        TriangleBatch<max_batch> batch;
        int batch_size= 0;
        for(int i= 0; i < mesh->triangleCount(); i++)
        {
            ...
            {
                triangles.push_back( batch );
                batch_size= 0;
            }
            
            batch.p[batch_size]= t.a;
            batch.edge1[batch_size]= gk::Point(t.b) - gk::Point(t.a);
            batch.edge2[batch_size]= gk::Point(t.c) - gk::Point(t.a);
            batch.color[batch_size]= gk::glsl::vec3 (1.f - float(i % 100) / 99.f, float(i % 10) / 9.f, float(i % 1000) / 999.f);
            batch_size++;
        }
        ...
        if(batch_size > 0)
        {
            ...
            {
                batch.p[batch_size]= gk::glsl::vec3();
                batch.edge1[batch_size]= gk::glsl::vec3();
                batch.edge2[batch_size]= gk::glsl::vec3();
                batch.color[batch_size]= gk::glsl::vec3 (1.f - float(i % 100) / 99.f, float(i % 10) / 9.f, float(i % 1000) / 999.f);
            }
    ...
    int draw( )
    {
        ...
        program->uniform("triangleCount")= 2900;
